...
Alternatively, you can update the Compose file's `build` configuration to create a container image from source, eg:
...
+    build:
+      context: .
+      dockerfile: Dockerfile.arm64  # or Dockerfile.armv7
```

You can then build and run the new container image using the following:
